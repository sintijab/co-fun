...
export const NavigationTop = ({
  ...
  links: { label: string; href: string; display?: FlexProps }[];
  onClick?: () => void;
  signInText?: string;
}) => {
  ...
  if (isIndex) {
    return (
      <Flex
        ...
        direction={['row', 'row']}
        p={['.5rem', '1.5rem 1.8rem']}
        zIndex={1}
        width={['100%']}
      >
        <Logo pl={['36px', 'unset']} margin="auto" height={['15px', '17px']} />
      </Flex>
    );
  }
  return (
    <Flex
      ...
      direction={['row', 'row']}
      p={['.5rem', '1.5rem 1.8rem']}
      boxShadow="0 3px 2px -2px rgba(32, 33, 36, 0.1)"
      zIndex={1000}
      width={['100%']}
    >
      <Box
      ...
      <Flex direction={['column', 'row']}>
        <Box
          ...
          pr="0"
          display={['block', 'none']}
          background={isOpen ? 'primary.green' : 'transparent'}
          borderRadius=".5rem"
        >
        ...
        >
          {links.map((item, i) => (
            ...
            <Link
              ...
              variant="nav"
              display="none"
              padding={['.7rem 2.5rem', '0 2rem']}
              textDecoration={
                item.href.includes(url) && !isHome ? 'underline' : 'none'
              }
            >
              {item.label}
            </Link>
