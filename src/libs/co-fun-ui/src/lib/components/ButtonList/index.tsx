...
export const ButtonList = ({ title, description, answers, showInfo = true, onClick, style }: { title: string; description?: string | string[]; showInfo?: boolean, answers: { answer: string; key: string }[],  onClick: (key: string) => void, style?: { display: string } }) => {
  ...
  return (
    <>
      ...
      <List spacing={1} {...style}>{answers.map((a, i) => {
        const parts = a.answer.split(':');
        if (parts.length > 1) {
          return <ListItem key={`button-list-${i}`}><ButtonPrimary variant="list" mb="2px" ml="2px" onClick={() => onClick(a.key)} flexDirection="column" lineHeight="1.5" alignItems="baseline" height="max-content">{parts[0]}<TextBase textStyle="button-select-descr">{parts[1]}</TextBase></ButtonPrimary></ListItem>
        }
        return <ListItem key={`button-list-${i}`}><ButtonPrimary variant="list" mb="2px" ml="2px" onClick={() => onClick(a.key)}>{a.answer}</ButtonPrimary></ListItem>
      })}
      </List>
    </>
  )
