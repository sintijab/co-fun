...
export const Footer = ({ columns, ...props }: { columns: { title: string; links: { label: string; href: string}[] }[] }) => (
  <Flex justifyContent="flex-start" gap="12%" flexWrap="wrap" {...props}>
    ...
    {columns.map((col, colIndex) => <Flex direction="column" key={`footer-col-` + colIndex}><TextBase pb="1rem" fontWeight="semibold" textStyle="footer">{col.title}</TextBase>{col.links.map(((link, linkIndex) => <Link key={`footer-link-` + linkIndex} href={`https://cofun.digital${link.href}`}><TextBase textStyle="footer" pb="0.75rem">{link.label}</TextBase></Link>))}</Flex>)}
  </Flex>
)