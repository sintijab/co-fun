    ...
    <Provider store={store}>
      <PersistGate persistor={persistor}>
        <CoFunUiProvider>
          ...
          {!isChat && (
            <TextBase textAlign="right" pr=".5rem">
              © 2024 Sintija Birgele
            </TextBase>
          )}
          {isIndex && isChatVisible && (
            <AspectRatio
              ratio={3 / 4}
              position="fixed"
              bottom="4rem"
          ...
          {isIndex && (
            <Box
              background="primary.white"
              onClick={() =>
                toggleWidget(isChatVisible === null ? true : !isChatVisible)
              }
              cursor="pointer"
              position="fixed"
              bottom="1rem"
